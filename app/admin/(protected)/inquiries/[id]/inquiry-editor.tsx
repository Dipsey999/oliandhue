...
import { PIPELINE_STATUSES, PRIORITIES, STATUS_COLORS, REQUIREMENT_LABELS } from '@/lib/constants'
...
import type { ProjectInquiry } from '@/lib/types/database'

interface InquiryEditorProps {
  inquiry: ProjectInquiry
}

export function InquiryEditor({ inquiry }: InquiryEditorProps) {
  ...
  const [company, setCompany] = useState(inquiry.company ?? '')

  // Project info fields
  const [requirement, setRequirement] = useState(inquiry.requirement ?? '')
  const [budget, setBudget] = useState(inquiry.budget ?? '')
  const [projectDetails, setProjectDetails] = useState(inquiry.project_details ?? '')
  const [projectLinks, setProjectLinks] = useState(inquiry.project_links ?? '')
  const [projectValue, setProjectValue] = useState(inquiry.project_value ?? '')
  const [priority, setPriority] = useState(inquiry.priority ?? 'medium')
  const [assignedTo, setAssignedTo] = useState(inquiry.assigned_to ?? '')
  const [followUpDate, setFollowUpDate] = useState(inquiry.follow_up_date ?? '')
  const [tags, setTags] = useState(inquiry.tags?.join(', ') ?? '')
  ...
  async function handleStatusChange(newStatus: string) {
    ...
    setStatus(newStatus as ProjectInquiry['status'])
    ...
    if (error) {
      setStatus(oldStatus as ProjectInquiry['status'])
  ...
  return (
    <div className="max-w-5xl space-y-6">
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="e.g. $5,000 - $10,000" className={inputClass} />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={projectLinks} onChange={(e) => setProjectLinks(e.target.value)} placeholder="Reference links" className={inputClass} />
            </div>
            <div>
            ...
            <div>
              ...
              <select
                value={priority}
                onChange={(e) => setPriority(e.target.value as ProjectInquiry['priority'])}
                className={inputClass}
              >
                {PRIORITIES.map((p) => (
      ...
      {/* Payments Section */}
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
        <PaymentManager entityType="inquiry" entityId={inquiry.id} />
      </div>

      {/* Timeline Section */}
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
        <h3 className="text-sm font-semibold text-white mb-4">Timeline</h3>
        <Timeline entityType="inquiry" entityId={inquiry.id} />
      </div>
      ...
      <div className="border border-red-900/30 rounded-xl p-6">
        ...
        <p className="text-xs text-neutral-500 mb-4">
          Permanently delete this inquiry and all associated data including payments and status history.
        </p>
        <button
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to permanently delete this inquiry? This will also remove all associated payments and status history. This action cannot be undone."
      />
    </div>
  )
}
